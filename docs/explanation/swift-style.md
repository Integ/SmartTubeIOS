...
- Continuation lines indented +4 from the original line.
...
- `ObservableObject` / `@Published` / Combine are not used anywhere in this codebase.

### 10.3 Dependency injection

- Pass dependencies through `init` parameters, not singletons.
- Type dependencies as protocols, not concrete types (see DIP in [solid.md](solid.md)).
