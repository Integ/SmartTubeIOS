...
public final class BotGuardWebViewRunner: NSObject {
    ...
    public func prepare(for videoId: String = "") async -> PrepareResult? {
        ...
        let task = Task<PrepareResult?, Never> { @MainActor [weak self] in
            ...
            if result.hasMinter {
                ...
                bgwvLog.notice(
                    "[BotGuardWV] ✅ prepare() succeeded — hasMinter=true integrityTokenLen=\(result.integrityTokenLen) ttl=\(result.ttl)s"
                )
                ...
                bgwvLog.notice(
                    "[BotGuardWV] ⚠️ prepare() — hasMinter=false (WAA returned null integrityToken; websafe fallback path)"
                )
            }
    ...
    public func mintToken(identifier: String) async -> String? {
        ...
        if let token {
            bgwvLog.notice(
                "[BotGuardWV] ✅ mintToken(identifier.len=\(effectiveIdentifier.count) webVD=\(self.webVisitorData.isEmpty ? "no" : "yes")) → token len=\(token.count)"
            )
        } else {
            bgwvLog.notice(
                "[BotGuardWV] ⚠️ mintToken(identifier.len=\(effectiveIdentifier.count) webVD=\(self.webVisitorData.isEmpty ? "no" : "yes")) → nil"
            )
        }
                ...
                for cookie in cookies {
                    guard
                        cookie.domain.contains("youtube.com") || cookie.domain.contains("google.com")
                            || cookie.domain.contains("googlevideo.com")
                    else { continue }
        ...
        webView.evaluateJavaScript(Self.pipelineJS(videoId: currentVideoId)) { [weak self] _, error in
            if let error {
                ...
                if msg.contains("unsupported type") || msg.contains("Promise") || msg.contains("JSSynchronousError") {
                    bgwvLog.notice(
                        "[BotGuardWV] pipeline JS injected (async IIFE — Promise return is expected; result via WKScriptMessageHandler)"
                    )
                    return
                }
...
extension BotGuardWebViewRunner: WKScriptMessageHandler {
    ...
    private func handlePrepareMessage(_ json: [String: Any]) {
        ...
        if success {
            bgwvLog.notice(
                "[BotGuardWV] prepare result: hasMinter=\(hasMinter) integrityTokenLen=\(integrityTokenLen) ttl=\(ttl)s websafeToken.len=\(websafeToken?.count ?? 0) webVisitorData.len=\(webVD.count)"
            )
        } else {
