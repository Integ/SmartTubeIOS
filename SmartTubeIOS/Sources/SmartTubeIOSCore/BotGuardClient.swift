...
public final class BotGuardClient: PoTokenProvider, @unchecked Sendable {
    ...
    private static let waaCreateURL = URL(
        string: "https://jnn-pa.googleapis.com/$rpc/google.internal.waa.v1.Waa/Create")!
    private static let waaGenerateITURL = URL(
        string: "https://jnn-pa.googleapis.com/$rpc/google.internal.waa.v1.Waa/GenerateIT")!
    ...
    public func token(for videoId: String) async throws -> String {
            ...
            let cached = cachedPoToken, let expiry = cachedPoTokenExpiry, Date() < expiry
        {
            ...
            bgLog.notice(
                "[BotGuard] ✅ cached PO token (ttl=\(ttlRemaining, privacy: .public)s) for \(videoId, privacy: .public)"
            )
            return cached
        }
        ...
        bgLog.notice(
            "[BotGuard] challenge ok, globalName=\(challenge.globalName, privacy: .public) jsLen=\(challenge.interpreterJS.count)"
        )

        // Phase 2–5 – run entirely on jsQueue to keep all JSValue references on one thread.
        let (token, ttl) = try await withCheckedThrowingContinuation {
            (cont: CheckedContinuation<(String, Int), Error>) in
            jsQueue.async {
                do {
    ...
    private func parseInnerArray(_ inner: [Any]) async throws -> BotGuardChallenge {
        ...
        if let urlRaw = wrappedUrl.compactMap({ $0 as? String }).first(where: { !$0.isEmpty }) {
            bgLog.notice(
                "[BotGuard] fetching interpreter JS from wrappedUrl: \(String(urlRaw.prefix(80)), privacy: .public)")
        ...
        for pattern in patterns {
                ...
                let range = Range(match.range(at: 1), in: html)
            {
                ...
                if path.hasPrefix("//") {
                    ...
                    fullURL = "https://www.youtube.com\(path)"
                }
        ...
        while pos < bytes.count {
            ...
            case 3:  // start group (proto2, deprecated) — skip until matching end group
                ...
                groupLoop: while pos < bytes.count && depth > 0 {
                    ...
                    switch iWire {
                    case 0:
                        guard let (_, p2) = readVarint(from: bytes, at: pos) else { break groupLoop }
                        pos = p2
                    case 1:
                        guard pos + 8 <= bytes.count else { break groupLoop }
                        pos += 8
                    case 2:
                    ...
                    case 5:
                        guard pos + 4 <= bytes.count else { break groupLoop }
                        pos += 4
    ...
    private func runPipelineSync(challenge: BotGuardChallenge, videoId: String) throws -> (token: String, ttl: Int) {
        ...
        ctx.evaluateScript(
            "try { if (typeof __bgSubtle !== 'undefined') { if (typeof crypto === 'undefined') { var crypto = {}; } crypto.subtle = __bgSubtle; } } catch(e) {}"
        )
        ...
        ctx.evaluateScript(
            "try { if (typeof __bgLog !== 'undefined') { if (typeof console === 'undefined') { var console = {}; } console.log = __bgLog; console.warn = __bgLog; console.error = __bgLog; } } catch(e) {}"
        )
        ...
        if vm == nil || vm!.isNull || vm!.isUndefined {
            ...
            if let name = discovered?.toString(), name != "null", !name.isEmpty {
                bgLog.notice(
                    "[BotGuard] discovered globalName=\(name, privacy: .public) (requested='\(challenge.globalName, privacy: .public)')"
                )
                vm = ctx.globalObject?.objectForKeyedSubscript(name)
            }
        }
        guard let vm, !vm.isNull, !vm.isUndefined else {
            throw BotGuardError.jsFailed(
                "VM '\(challenge.globalName)' not in JSContext global (dynamic discovery also failed)")
        }
        ...
        let vmCallResult = vm.invokeMethod(
            "a",
            withArguments: [
                challenge.program,
        ...
        bgLog.notice(
            "[BotGuard] Phase 4 ✅ integrityToken=\(integrityToken?.count ?? 0) websafeFallback=\(websafeFallback?.count ?? 0)"
        )
    ...
    private func fetchIntegrityTokenSync(
        bgResponse: String
    ) throws -> (integrityToken: String?, websafeFallback: String?, ttl: Int) {
        ...
        session.dataTask(with: req) { data, response, error in
            ...
            guard let data,
                ...
                box.value = .failure(
                    BotGuardError.integrityTokenFailed(
            ...
            log.notice(
                "[BotGuard] GenerateIT: integrityToken=\(integrityToken?.count ?? 0) websafeFallback=\(websafeFallback?.count ?? 0)"
            )
            guard integrityToken != nil || websafeFallback != nil else {
                box.value = .failure(
                    BotGuardError.integrityTokenFailed("both integrityToken and websafeFallback are nil"))
                return
            }
    ...
    private func mintSync(
        ...
        let mintCallbackFn = try resolvePromise(
            getMinterResult ?? JSValue(undefinedIn: ctx)!, in: ctx, label: "getMinter")
    ...
    private func installPolyfills(_ ctx: JSContext) {
        ...
        let hmacFn: @convention(block) (JSValue, JSValue) -> JSValue = { keyArr, dataArr in
            ...
            bgLogCapture.info(
                "[BotGuard] __bgCrypto_hmac keyLen=\(kl, privacy: .public) dataLen=\(dl, privacy: .public)")
    ...
    private func resolvePromise(
        _ promise: JSValue, in ctx: JSContext, label: String, maxPumps: Int = 50
    ) throws -> JSValue {
